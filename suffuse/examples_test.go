...
func ExampleSed() {
  with(func (in string, out string) {
    execute := executeIn(out)

    // Fill a file with digits 1 to 10.
    pipeToFile(command("seq", "1", "10"), in + "/seq.txt")
...
func pipeToFile(cmd *exec.Cmd, file string) error {
